...
{
  nvidia_nv40 = subdevice<geforce_7600gs_device>("pci:01.0:00.0")->debug_get_renderer();
}
...
{
	pentium4_device &maincpu(PENTIUM4(config, "maincpu", 28000000U*5)); /* Actually Celeron D at 2,8 GHz */
  maincpu.set_irq_acknowledge_callback("pci:1f.0:pic_master", FUNC(pic8259_device::inta_cb));

  screen_device &screen(SCREEN(config, "screen", SCREEN_TYPE_RASTER));
  screen.set_raw(XTAL(25'174'800), 900, 0, 640, 526, 0, 480);
  screen.set_screen_update(m_vga, FUNC(nvidia_g73_vga_device::screen_update));

  NVIDIA_G73_VGA(config, m_vga, 0);
  m_vga->set_screen("screen");
  m_vga->set_vram_size(512*1024*1024);

	PCI_ROOT                (config, "pci",           0);
	I82875P_HOST            (config, "pci:00.0",      0,                   0x103382c0, "maincpu", 512*1024*1024);
	I82875P_AGP             (config, "pci:01.0",      0);
	GEFORCE_7600GS          (config, "pci:01.0:00.0", 0,                   0x10de02e1, m_vga,     "maincpu");
	...
	I6300ESB_LPC            (config, "pci:1f.0",      0,                   "maincpu");
...
#define LINDBERGH_BIOS \
  ...
  ROM_SYSTEM_BIOS(3, "bios3", "Coreboot") \
  ROMX_LOAD("coreboot.rom",  0x00000, 0x100000, CRC(7df7984c) SHA1(95a59dad6e66000c1253563df4eb75a3d698b2ab), ROM_BIOS(3) ) \
	\
