...
class mc97_device : public pci_device {
public:
	mc97_device(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock, uint32_t main_id, uint32_t revision, uint32_t subdevice_id)
		: mc97_device(mconfig, tag, owner, clock)
	{
		set_ids(main_id, revision, 0x070300, subdevice_id);
	}
	mc97_device(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock);
...
DECLARE_DEVICE_TYPE(MC97, mc97_device)
