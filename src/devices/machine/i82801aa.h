...
// Intel i82801AA southbridge

#ifndef MAME_MACHINE_I82801AA_H
#define MAME_MACHINE_I82801AA_H
...
class i82801aa_lpc_device : public pci_device {
public:
	template <typename T>
	i82801aa_lpc_device(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock, T &&cpu_tag)
		: i82801aa_lpc_device(mconfig, tag, owner, clock)
	{
		set_cpu_tag(std::forward<T>(cpu_tag));
	}

	i82801aa_lpc_device(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock);
...
class i82801aa_ide_device : public pci_device
{
public:
	template <typename T>
	i82801aa_ide_device(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock, T &&cpu_tag)
		: i82801aa_ide_device(mconfig, tag, owner, clock)
	{
		set_cpu_tag(std::forward<T>(cpu_tag));
	}

	i82801aa_ide_device(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock);
...
protected:
	i82801aa_ide_device(const machine_config &mconfig, device_type type, const char *tag, device_t *owner, uint32_t clock);
...
DECLARE_DEVICE_TYPE(I82801AA_LPC,      i82801aa_lpc_device)
DECLARE_DEVICE_TYPE(I82801AA_IDE, 	   i82801aa_ide_device)

#endif // MAME_MACHINE_I82801AA_H
