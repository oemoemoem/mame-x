...
// Intel i82820 northbridge

#ifndef MAME_MACHINE_I82820_H
#define MAME_MACHINE_I82820_H

#pragma once

#include "pci.h"

class i82820_host_device : public pci_host_device {
public:
	template <typename T>
	i82820_host_device(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock, uint32_t subdevice_id, T &&cpu_tag, int ram_size)
		: i82820_host_device(mconfig, tag, owner, clock)
	{
		set_ids_host(0x80862500, 0x03, subdevice_id);
		set_cpu_tag(std::forward<T>(cpu_tag));
		set_ram_size(ram_size);
	}

	i82820_host_device(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock);
...
class i82820_agp_device : public agp_bridge_device {
public:
	i82820_agp_device(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock);
...
DECLARE_DEVICE_TYPE(I82820_HOST,     i82820_host_device)
DECLARE_DEVICE_TYPE(I82820_AGP,      i82820_agp_device)

#endif // MAME_MACHINE_I82820_H
