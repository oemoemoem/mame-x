...
public:
	template <typename T>
	i6300esb_lpc_device(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock, T &&cpu_tag)
		: i6300esb_lpc_device(mconfig, tag, owner, clock)
	{
...
class i6300esb_ide_device : public pci_device
{
public:
	template <typename T>
	i6300esb_ide_device(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock, T &&cpu_tag)
		: i6300esb_ide_device(mconfig, tag, owner, clock)
	{
		set_cpu_tag(std::forward<T>(cpu_tag));
	}

	i6300esb_ide_device(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock);
...
protected:
	i6300esb_ide_device(const machine_config &mconfig, device_type type, const char *tag, device_t *owner, uint32_t clock);
...
DECLARE_DEVICE_TYPE(I6300ESB_IDE, 	   i6300esb_ide_device)
