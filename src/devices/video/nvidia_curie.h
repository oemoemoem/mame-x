...
/*
 * geforce 3d (NV40) vertex program disassembler
 */
#ifndef MAME_VIDEO_NVIDIA_CURIE_H
#define MAME_VIDEO_NVIDIA_CURIE_H

#pragma once

#include "video/poly.h"

class nv40_vertex_program_disassembler {
...
public:
	nv40_vertex_program_disassembler() { state = 0; }
	int disassemble(unsigned int *instruction, char *line);
};

/*
 * geforce 3d (NV40) vertex structure
 */
struct vertex_nv40 {
	union {
		float fv[4];
...
/*
 * geforce 3d (NV40) vertex program simulator
 */
class nv40_vertex_program_simulator {
public:
...
public:
	void set_data(vertex_nv40 *in, vertex_nv40 *out);
	void reset();
	int step();
	void decode_instruction(int address);
	void execute();
	void jump(int address);
	void process(int address, vertex_nv40 *in, vertex_nv40 *out, int count);
	int status();
private:
...
class nv40_renderer; // forward declaration
struct nv40_object_data
{
	nv40_renderer *data;
};

/*
 * geforce 3d (NV40) accelerator
 */
...
class nv40_rasterizer : public poly_manager<double, nv40_object_data, 26>
{
public:
	nv40_rasterizer(running_machine &machine) : poly_manager<double, nv40_object_data, 26>(machine)
	{
	}
};
...
public:
	...
	enum class NV40_BLEND_EQUATION {
		FUNC_ADD = 0x8006,
		MIN = 0x8007,
		MAX = 0x8008,
	...
	enum class NV40_RT_TYPE {
		LINEAR = 1,
		SWIZZLED = 2
	};
	enum class NV40_RT_DEPTH_FORMAT {
		Z16 = 0x0001,
		Z24S8 = 0x0002
	};
	enum class NV40_COLOR_FORMAT {
		X1R5G5B5_Z1R5G5B5 = 1,
		X1R5G5B5_X1R5G5B5 = 2,
		R5G6B5 = 3,
		X8R8G8B8_Z8R8G8B8 = 4,
	...
	enum class NV40_GL_FRONT_FACE {
		CW = 0x0900,
		CCW = 0x0901
	};
	enum class NV40_GL_CULL_FACE {
		FRONT = 0x0404,
		BACK = 0x0405,
	...
	struct NV40vertex_t : public nv40_rasterizer::vertex_t
	{
		double w;
	};

	nv40_renderer(running_machine &machine)
		: rasterizer(machine)
		, mach(machine)
	{
		...
		backface_culling_winding = NV40_GL_FRONT_FACE::CCW;
		backface_culling_culled = NV40_GL_CULL_FACE::BACK;
		alpha_test_enabled = false;
		alpha_reference = 0;
		alpha_func = NV40_COMPARISON_OP::ALWAYS;
		depth_test_enabled = false;
		depth_function = NV40_COMPARISON_OP::LESS;
		depth_write_enabled = false;
		stencil_test_enabled = false;
		stencil_func = NV40_COMPARISON_OP::ALWAYS;
		stencil_ref = 0;
		stencil_mask = -1;
		stencil_op_fail = NV40_STENCIL_OP::KEEP;
		stencil_op_zfail = NV40_STENCIL_OP::KEEP;
		stencil_op_zpass = NV40_STENCIL_OP::KEEP;
		blending_enabled = false;
		blend_equation = NV40_BLEND_EQUATION::FUNC_ADD;
		blend_color = 0;
		blend_function_destination = NV40_BLEND_FACTOR::ZERO;
		blend_function_source = NV40_BLEND_FACTOR::ONE;
		logical_operation_enabled = false;
		logical_operation = NV40_LOGIC_OP::COPY;
		fog_color = 0;
		for (int n = 0; n < 4; n++) {
			texture[n].enabled = 0;
			texture[n].mode = 0;
		...
		depthformat_rendertarget = NV40_RT_DEPTH_FORMAT::Z24S8;
		colorformat_rendertarget = NV40_COLOR_FORMAT::A8R8G8B8;
		bytespixel_rendertarget = 4;
		clear_rendertarget.set(0, 0, 639, 479);
		primitive_type = NV40_BEGIN_END::STOP;
		antialias_control = 0;
	...
	void render_texture_simple(int32_t scanline, const nv40_rasterizer::extent_t &extent, const nv40_object_data &extradata, int threadid);
	void render_color(int32_t scanline, const nv40_rasterizer::extent_t &extent, const nv40_object_data &extradata, int threadid);
	void render_register_combiners(int32_t scanline, const nv40_rasterizer::extent_t &extent, const nv40_object_data &objectdata, int threadid);
	...
	float combiner_map_input_select(int id, Combiner1::InputRegister code, int index); // get component index in register selected as input
	float *combiner_map_input_select_array(int id, Combiner1::InputRegister code); // get pointer to register selected as input
	float *combiner_map_output_select_array(int id, Combiner1::InputRegister code); // get pointer to register selected as output
	float combiner_map_input_function(Combiner1::MapFunction code, float value); // apply input mapping function code to value
	void combiner_map_input_function_array(Combiner1::MapFunction code, float *data); // apply input mapping function code to data
	...
	void debug_grab_nv40_vertex_program_slot(int slot, uint32_t *instruction);
	...
	void read_vertex(address_space &space, offs_t address, vertex_nv40 &vertex, int attrib);
	...
	void convert_vertices(vertex_nv40 *source, NV40vertex_t *destination);
	void assemble_primitive(int source, int count);
	void process_persistent_vertex();
	int clip_triangle_w(NV40vertex_t vi[3], NV40vertex_t *vo);
	uint32_t render_triangle_clipping(const rectangle &cliprect, NV40vertex_t &_v1, NV40vertex_t &_v2, NV40vertex_t &_v3);
	uint32_t render_triangle_culling(const rectangle &cliprect, NV40vertex_t &_v1, NV40vertex_t &_v2, NV40vertex_t &_v3);
	...
	NV40_RT_TYPE type_rendertarget;
	NV40_RT_DEPTH_FORMAT depthformat_rendertarget;
	NV40_COLOR_FORMAT colorformat_rendertarget;
	...
	vertex_nv40 vertex_software[1024+2]; // vertex attributes sent by the software to the 3d accelerator
	NV40vertex_t vertex_xy[1024+2]; // vertex attributes computed by the 3d accelerator
	vertex_nv40 persistvertexattr; // persistent vertex attributes
	nv40_rasterizer::render_delegate render_spans_callback;
	Combiner1 combiner;
	uint32_t color_mask;
	bool backface_culling_enabled;
	NV40_GL_FRONT_FACE backface_culling_winding;
	NV40_GL_CULL_FACE backface_culling_culled;
	...
	NV40_COMPARISON_OP alpha_func;
	int alpha_reference;
	NV40_COMPARISON_OP depth_function;
	NV40_COMPARISON_OP stencil_func;
	int stencil_ref;
	int stencil_mask;
	NV40_STENCIL_OP stencil_op_fail;
	NV40_STENCIL_OP stencil_op_zfail;
	NV40_STENCIL_OP stencil_op_zpass;
	NV40_BLEND_EQUATION blend_equation;
	NV40_BLEND_FACTOR blend_function_source;
	NV40_BLEND_FACTOR blend_function_destination;
	uint32_t blend_color;
	NV40_LOGIC_OP logical_operation;
	uint32_t fog_color;
	struct {
		float modelview[4][4];
	...
	struct {
		nv40_vertex_program_simulator exec;
		int instructions;
	...
	int dilatechose[256]{};
	nv40_object_data *objectdata = nullptr;
...
#endif // MAME_VIDEO_NVIDIA_CURIE_H
