...
DEFINE_DEVICE_TYPE(NVIDIA_G73_VGA,  nvidia_g73_vga_device,  "nvidia_g73_vga",  "nVidia G73 VGA i/f")

nvidia_g73_vga_device::nvidia_g73_vga_device(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock)
	: svga_device(mconfig, NVIDIA_G73_VGA, tag, owner, clock)
{
}

void nvidia_g73_vga_device::device_add_mconfig(machine_config &config)
{
	// TODO: I2C, likely for DDC (AIDA16 extensively test that in non-safe mode)
}

void nvidia_g73_vga_device::device_start()
{
	svga_device::device_start();
	zero();
	...
	vga.svga_intf.seq_regcount = 0x05;
	vga.svga_intf.crtc_regcount = 0x40;
	vga.svga_intf.vram_size = 256*1024*1024;
...
// TODO: should really calculate the access bit internally
uint8_t nvidia_g73_vga_device::port_03b0_r(offs_t offset)
{
	uint8_t res = 0xff;
...
void nvidia_g73_vga_device::port_03b0_w(offs_t offset, uint8_t data)
{
	if (get_crtc_port() == 0x3b0)
	{
		switch(offset)
...
uint8_t nvidia_g73_vga_device::port_03d0_r(offs_t offset)
{
	uint8_t res = 0xff;
...
void nvidia_g73_vga_device::port_03d0_w(offs_t offset, uint8_t data)
{
	if (get_crtc_port() == 0x3d0)
	{
		switch(offset)
...
uint8_t nvidia_g73_vga_device::mem_r(offs_t offset)
{
	if (svga.rgb8_en || svga.rgb15_en || svga.rgb16_en || svga.rgb24_en || svga.rgb32_en)
		return svga_device::mem_linear_r(offset + (svga.bank_r * 0x8000));
	return svga_device::mem_r(offset);
...
void nvidia_g73_vga_device::mem_w(offs_t offset, uint8_t data)
{
	if (svga.rgb8_en || svga.rgb15_en || svga.rgb16_en || svga.rgb24_en || svga.rgb32_en)
	{
		svga_device::mem_linear_w(offset + (svga.bank_w * 0x8000), data);
...
uint16_t nvidia_g73_vga_device::offset()
{
	if (svga.rgb8_en || svga.rgb16_en || svga.rgb24_en || svga.rgb32_en)
		return (vga.crtc.offset << 3) + m_ext_offset;
	return svga_device::offset();
...
uint8_t nvidia_g73_vga_device::crtc_reg_read(uint8_t index)
{
	uint8_t res = 0;
	...
	switch(index)
	{
		...
		case 0x38:
			...
			break;
		case 0x3e:
			res = (g73.i2c.sda << 3) | (g73.i2c.scl << 2);
			break;
		default:
			LOGCRTC("EXT CRTC: %02x Read\n", index);
			//machine().debug_break();
...
void nvidia_g73_vga_device::crtc_reg_write(uint8_t index, uint8_t data)
{
	//index &= 0x3f;
	if(index <= 0x18)
	{
	...
	switch(index)
	{
		...
		case 0x38:
			...
			break;
		case 0x3f:
			g73.i2c.scl = data & 0x20;
			g73.i2c.sda = data & 0x10;
			break;
		default:
